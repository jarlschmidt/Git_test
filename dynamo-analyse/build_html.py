...
<div class="insight"><b>Historie-niveau er komplet for 2015–2026:</b> alle {STORY_ISSUE_COUNT} numre i perioden (nr. {min(STORY_ISSUE_NUMS)}–{max(STORY_ISSUE_NUMS)}) er nu historie-udtrukket — de sidste fem (nr. 41, 83, 84, 85, 86), som issuu's tekstlag-API afviste eller som lå på en siden nedlagt visningsplatform, blev læst direkte fra PDF'er brugeren selv sendte. Den resterende begrænsning er 2005–2014: her er {sum(1 for i in issues if i["year"]<=2014 and i.get("confidence")=="not_found")} af {sum(1 for i in issues if i["year"]<=2014)} numre fortsat udokumenterede efter udvidet søgning i DTU's mediebibliotek — et par lovende kilder (yumpu.com, Wayback Machine) var blokeret af netværksproxyen i analysemiljøet. Se dækningsgraden nedenfor.</div>
<div class="cols">
...
<p>Denne rapport er udarbejdet i to lag: et issue-niveau (forsidetema, udgivelsesår, kilde) for alle {TOTAL} numre, katalogiseret ud fra <b>issuu.com/dtudk</b> og de originale PDF'er i <b>DTU's mediebibliotek (dtu.dk)</b> — og et historie-niveau ({STORY_COUNT_DA} enkeltstående historier med titel, institut og emne) for <b>alle {STORY_ISSUE_COUNT} numre fra 2015–2026</b> (nr. {min(STORY_ISSUE_NUMS)}–{max(STORY_ISSUE_NUMS)}), enten udtrukket fra issuu.com's interne søgeindeks eller — for de fem numre issuu afviste — læst direkte fra PDF'er brugeren selv sendte (se metodeafsnittet). Hovedparten af 2005–2014 har fortsat kun issue-niveau-dokumentation. Kildelinks til hvert enkelt nummer findes i appendiks A; alle historier findes i appendiks B.</p>
