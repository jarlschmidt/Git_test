...
- **`Dynamo_gennem_20_aar.pdf`** — den færdige, søgbare rapport (67 sider, McKinsey-inspireret layout, DTU's officielle farver). To dokumentationsniveauer: issue-niveau (forsidetema) for alle 86 numre, og historie-niveau (hver enkelt artikel, med institut og emne) for 1.112 historier i **alle 46 numre 2015–2026** — hvor magasinets fulde tekst kunne udtrækkes fra issuu eller blev uploadet direkte af brugeren.
...
- `data/stories.json` — historie-niveau katalog: 1.112 enkeltstående historier (nummer, år, titel, institut, emne) fra **alle 46 numre 2015–2026**, primært udtrukket via `extract_issuu_text.py`; nr. 41, 83, 84, 85 og 86 er struktureret direkte fra brugerens uploadede PDF'er (pdftotext), da issuu's API afviste dem eller (nr. 41) fordi den oprindelige visningsplatform ikke længere findes.
...
- Historie-niveau er nu komplet for **alle 46 numre** i perioden 2015–2026. De sidste fem
  (41, 83, 84, 85, 86) kunne ikke historie-udtrækkes via issuu — enten fordi API'en afviste dem
  (403) eller fordi nr. 41 lå på en visningsplatform, der ikke længere findes
  (emagstudio.win.dtu.dk) — men blev struktureret direkte fra PDF'er, brugeren selv uploadede
  (2026-09-07). Se `data/stories.json` (issue_number 41/83/84/85/86).
...
- Institut-tagging på issue-niveau (alle 86 numre) er stadig sparsom, da forsidetemaer
  ...
  rigere billede: se institutafsnittet i rapporten for den aktuelle andel og fordeling.
